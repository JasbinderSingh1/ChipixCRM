...
# IST Live Clock (autorefresh every second)
ist = pytz.timezone('Asia/Kolkata')
current_time_placeholder = st.empty()

# This live update will continue on rerun
current_time_placeholder.markdown(f"### 🕒 Current IST Time: {datetime.now(ist).strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if submitted:
        if validate_inputs() and all(details.values()):
            ...
            if hasattr(response, "data") and response.data:
                ...
                with st.expander("🔍 Search Entry"):
                    query = st.text_input("Search by Name or Phone", value=phone)
                    if query:
                        result = supabase.table("chipix_customers").select("*").ilike("phone", f"%{query}%").execute()
                        record = result.data[0] if result.data else None
                        if record:
                            st.markdown(f"### 🎫 Entry Found for {record['name']}")
